...
public class BLEduinoUartActivity extends Activity {
	...
	private final static int WSTATE_NOTIFY_BLEDUINO=2;
	private final static int WSTATE_READ_BLEDUINO=3;
	private final static int WSTATE_DUMMY=4;
	private final static int WSTATE_WRITE_BLEDUINO=5;
		...
		protected void nextState(){
			switch(mState) {			   
			   ...
			   case(WSTATE_SEARCH_SERVICES):
			       ...
			       break;			   
			   case(WSTATE_READ_BLEDUINO):
			       if (Constant.DEBUG) Log.i("JMG","State Read Key");
			   	   mTumakuBLE.read(TumakuBLE.BLEDUINO_UART_SERVICE,TumakuBLE.BLEDUINO_UART_TX);
				   break;
			   case(WSTATE_NOTIFY_BLEDUINO):
			       if (Constant.DEBUG) Log.i("JMG","State Notify Key");
		   	       mTumakuBLE.enableNotifications(TumakuBLE.BLEDUINO_UART_SERVICE,TumakuBLE.BLEDUINO_UART_TX,true);
				   break;

			   case(WSTATE_WRITE_BLEDUINO):
			       ...
			       for (int i=0; i<tmpString.length();i++) tmpArray[i]=(byte)tmpString.charAt(i);
		   	       mTumakuBLE.write(TumakuBLE.BLEDUINO_UART_SERVICE,TumakuBLE.BLEDUINO_UART_RX, tmpArray);
				   break;
    ...
    private class HM10BroadcastReceiver extends BroadcastReceiver {
       ...
       public void onReceive(Context context, Intent intent) {
           ...
           if (intent.getAction().equals(TumakuBLE.READ_SUCCESS)) {
        	   ...
        	   if (mState==WSTATE_READ_BLEDUINO) {
           ...
           if (intent.getAction().equals(TumakuBLE.WRITE_SUCCESS)) {
        	   ...
        	   if (mState==WSTATE_WRITE_BLEDUINO) {
        		   mState=WSTATE_DUMMY;
        		   nextState();
        		   return;
        	   }    
           ...
           if (intent.getAction().equals(TumakuBLE.NOTIFICATION)) {
		       ...
		       if (!notificationValue.equalsIgnoreCase("null")) {
		    	   if (characteristicUUID.equalsIgnoreCase(TumakuBLE.BLEDUINO_UART_TX)) {
		 		       if (Constant.DEBUG) Log.i("JMG","NOTIFICATION of BLEduino UART Service");
           ...
           if (intent.getAction().equals(TumakuBLE.WRITE_DESCRIPTOR_SUCCESS)) {
        	   ...
        	   if (mState==WSTATE_NOTIFY_BLEDUINO) {
        		   mState=WSTATE_DUMMY;
        		   nextState();
        	   }
        	   return;
